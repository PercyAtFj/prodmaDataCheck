 ...
 * Date: 2012-08-28
 * 
 * 20130709： 在query前新增ts.p.postData.queryTotal标識，true表示需要查询count，
 * false 表示不查询total，默认从之前缓存中读取{"totalCount":23,"totalPage":3}
 * 
 * 
 */
//jsHint options
...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		var p = $.extend(true,{
			...
			aTagIntercept:false, //a标签拦截标志，默认为false，表示拦截，他不会触发onclick和onselect事件，设置为true的时候表示不拦截,会触发onclick和onselect
			onSelectRow: null,
			onClickRow:null,
			onSortCol: null,
			ondblClickRow: null,
			...
			onPaging: null,
		//	afterLoaded:null,//当分页组件发生变化时触发该事件
			onSelectAll: null,
			loadComplete: null,
			onLoadSuccess:null,//为了与easyui使用相似
			gridComplete: null,
			loadError: null,
			loadBeforeSend: null,
			afterInsertRow: null,
			...
			loadonce: false,
			multiselect: true,
			multiselectCondition:null, //多选框的check属性控制方法
			multikey: false,
			clickUnselect:true, //ture 表示点击和选中分离
			complexGroupHeaders:false,
			editurl: null,
			search: false,
			caption: "",
			hidegrid: false,//向上收缩
			...
			ExpandColumn: null,
			tree_root_level : 0,
			prmNames: {page:"page",rows:"rows", sort: "sort",order: "order", search:"_search", nd:"nd", id:"id",oper:"oper",editoper:"edit",addoper:"add",deloper:"del", subgridid:"id", npage: null, totalrows:"totalrows"},
			forceFit : false,
			gridstate : "visible",
			cellEdit: false,
			cellsubmit: "remote",
			...
			scrollrows : false,
			autowidth: true,//默认是false
			detailAndTotal:true,//默认同时查detail和total
			scrollOffset :18,
			cellLayout: 5,
			subGridWidth: 20,
			multiselectWidth: 20,
		...
		if(!p.rowList){
			p.rowList=[5,10,20,30,40,50];
		}
		if((!p.mytoolbar || p.mytoolbar.length == 0) && p.toolbar){ //没有工具栏时设定工具栏标志为false
			p.toolbar[0] = false;
		}
		var ts= this, grid={
			headers:[],
		...
		var eg = $("<div class='ui-jqgrid ui-widget ui-widget-contenta ui-corner-all'></div>");//ui-widget-content改为ui-widget-contenta
		...
		addMulti = function(rowid,pos,irow,checked){
			var attr = ts.p.multiselectCondition?ts.p.multiselectCondition.call(ts,rowid):"";
			var	v = "<input role=\"checkbox\" type=\"checkbox\""+attr+" id=\"jqg_"+ts.p.id+"_"+rowid+"\" class=\"cbox\" name=\"jqg_"+ts.p.id+"_"+rowid+"\"" + ((checked) ? "checked=\"checked\"" : "")+"/>",
		...
		constructTr = function(id, hide, altClass, rd, cur, selected,i) {
			...
			}else{//添加ui-widget-content_blue样式
			return '<tr role="row" id="' + id + '" tabindex="' + tabindex + '" class="' + classes + ' ui-widget-content_blue"' +
				(style === '' ? '' : ' style="' + style + '"') + restAttr + '>';
			}
		},
		addXmlData = function (xml,t, rcnt, more, adjust) {
			...
			while (j<gl) {
				...
				rowData[iStartTrTag] = constructTr(rid, hiderow, cn1, rd, xmlr, false,i);
				rowData.push("</tr>");
				if(ts.p.grouping) {
		...
		addJSONData = function(data,t, rcnt, more, adjust) {
			...
			while (i<len) {
				...
				rowData[iStartTrTag] = constructTr(idr, hiderow, cn1, rd, cur, selr, i);//为了添加样式
				rowData.push( "</tr>" );
				if(ts.p.grouping) {
...
//			}
		...
		endReq = function() {
			...
			ts.p.postData.queryTotal = false;
			switch(ts.p.loadui) {
				case "disable":
					break;
				case "enable":
		...
		populate = function (npage) {
			if(!ts.grid.hDiv.loading) {
				...
				var lcf2 = $.isFunction(ts.p.onLoadSuccess), lc2 = lcf2 ? ts.p.onLoadSuccess : null;
				var adjust = 0;
				npage = npage || 1;
				if (npage > 1) {
					if(pN.npage !== null) {
						...
						lc2 = function(req) {
							...
							ts.grid.hDiv.loading = false;
							if (lcf2) {
								ts.p.onLoadSuccess.call(ts,req);
							}
							populate(npage-1);
						};
					}
				...
				if(!ts.p.postData.queryTotal){
					prm["totalCount"] = ts.p.records;
				}
				...
				case "xml":
				case "script":
					if(ts.p.detailAndTotal || !ts.p.postData.queryTotal){//同时查或不需要查总数
						$.ajax($.extend({
							url:ts.p.url,
							type:ts.p.mtype,
							dataType: dt ,
							...
							success:function(data,st, xhr) {
								if(!data.totalCount && !ts.p.postData.queryTotal){
									data[ts.p.jsonReader.records] = prm.totalCount;
									data[ts.p.jsonReader.total] = Math.ceil(prm.totalCount / parseInt(ts.p.rowNum,10));
								}
								...
								////lixj 新增onLoadErrorMsg 事件////
								if($.isFunction(ts.p.onLoadErrorMsg) && typeof(data.data) !== "undefined" && typeof(data.data.succeed) !== "undefined" && !data.data.succeed){
										ts.p.onLoadErrorMsg.call(ts,data.data);
										endReq();
										return;
								}
							////lixj 新增onLoadErrorMsg 事件////
						...
						var postData = $.extend({},ts.p.postData,{"queryDetail":true,"queryTotal":false}),totalCount,maxWait=1000,userdata;
						$.ajax($.extend({
							url:ts.p.url,
							type:ts.p.mtype,
							dataType: dt ,
							data: $.isFunction(ts.p.serializeGridData)? ts.p.serializeGridData.call(ts,postData) : postData,
							success:function(data,st, xhr) {
								global.doSleep(function(){
									maxWait--;
									if(!totalCount && totalCount !=0 && maxWait > 0) return false;
									data[ts.p.jsonReader.records] = totalCount;
									data[ts.p.jsonReader.total] = Math.ceil(totalCount / parseInt(ts.p.rowNum,10));
									
									//add by liuxiaobin begin 2014-05-27
									data[ts.p.jsonReader.userdata] = data[ts.p.jsonReader.userdata]||{};
									$.extend(data[ts.p.jsonReader.userdata],userdata);
									//end
						...
						postData = $.extend({},ts.p.postData,{"queryDetail":false,"queryTotal":true});
						$.ajax($.extend({
							url:ts.p.url,
							type:ts.p.mtype,
							dataType: dt ,
							...
							success:function(data,st, xhr) {
								...
								userdata = data.userdata || {};//add by liuxiaobin 2014-05-27
							},
							error:function(xhr,st,err){
								if($.isFunction(ts.p.loadError)) { ts.p.loadError.call(ts,xhr,st,err); }
								if (npage === 1) { endReq(); }
				...
				case "xmlstring":
					...
					if(lcf2) {ts.p.onLoadSuccess.call(ts,dstr);}
		...
		setPager = function (pgid, tp){
			...
			$("#"+$.jgrid.jqID(pgid) )
			.append("<div id='"+pgcnt+"' class='ui-pager-control' role='group'><table cellspacing='0' cellpadding='0' border='0' class='ui-pg-table' style='width:100%;table-layout:fixed;height:100%;' role='row'><tbody><tr><td id='"+cent+"' align='left' style='float:right;display:none;'></td><td id='"+rgt+"' align='right' style='width:auto;float: right;'></td><td id='"+lft+"' align='center' style='white-space:pre;float:right;width:auto;'></td></tr></tbody></table></div>")
			.attr("dir","ltr"); //explicit setting
			if(ts.p.rowList.length >0){
				str = "<td dir='"+dir+"'>";
				str +="<select class='ui-pg-selbox' role='listbox' "+ts.p.rowListDisabled+">";
			...
			if(dir=="rtl") { pgl += str; }
			if(dir=="ltr") { pgl += str; }// qsl修改，放这个位置
			if(ts.p.pginput===true) { pginp= "<td dir='"+dir+"'>"+ts.p.pginputpre+$.jgrid.format(ts.p.pgtext || "","<input class='ui-pg-input' type='text' size='2' maxlength='7' value='0' role='textbox'/>" + ts.p.pginputpro,"<span id='sp_1_"+$.jgrid.jqID(pgid)+"'></span>")+"</td>";}
			if(ts.p.pgbuttons===true) {
			...
			//if(dir=="ltr") { pgl += str; }// qsl修改，放到上边的位置
			pgl += "</tr></tbody></table>";
		...
		sortData = function (index, idxcol,reload,sor){
			...
			if(!reload) {
				...
				} else { ts.p.sortorder = ts.p.colModel[idxcol].firstsortorder || /*'asc'*/ 'desc'; } // modify by liuxiaobin 2013-9-9 默认倒序
				ts.p.page = 1;
			}
			if(sor) {
		...
		setColWidth = function () {
			 ...
			 * 上行注释是原语句
			 */
			var brd=ts.p.cellLayout, vc=0, lvc, scw=intNum(ts.p.scrollOffset,0),cw,hs=false,aw,gw=0,
		...
		if(this.p.multiselect) {
			...
			var v = !this.p.multiselectCondition?"":"disabled='true'";
			this.p.colNames.unshift("<input role='checkbox' id='cb_"+this.p.id+"' class='cbox' type='checkbox' "+v+"/>");
		...
		if(this.p.multiselect && this.p.checkboxSingle) { //修改lixj，当选中框为单选时，disable全选框
			$('#cb_'+$.jgrid.jqID(ts.p.id),this).attr("disabled","true");
		}
		if(this.p.multiselect && !this.p.checkboxSingle) { //修改lixj，当选中框为单选时，不绑定全选事件
			var emp=[], chk;
			...
			}).bind('checkForallGrid',function(){//lixj
				var rowIdRec = $.makeArray(arguments).slice(1);
				ts.p.selarrrow = [];
				var froz = ts.p.frozenColumns === true ? ts.p.id + "_frozen" : "";
				chk=true;//默认全选
					$(ts.rows).each(function(i) {
						if (i>0) {
							if (rowIdRec && $.inArray(this.id,rowIdRec) < 0) {  //当不在排除列中
								if(!$(this).hasClass("ui-subgrid") && !$(this).hasClass("jqgroup") && !$(this).hasClass('ui-state-disabled')){
									$("#jqg_"+$.jgrid.jqID(ts.p.id)+"_"+$.jgrid.jqID(this.id) )[ts.p.useProp ? 'prop': 'attr']("checked",true);
									$(this).addClass("ui-state-highlight").attr("aria-selected","true");  
									ts.p.selarrrow.push(this.id);
			...
			}).bind("uncheckForallGrid",function(){
				var rowIdRec = $.makeArray(arguments).slice(1);
				ts.p.selarrrow = [];
				var froz = ts.p.frozenColumns === true ? ts.p.id + "_frozen" : "";
				chk=true;//默认全选
					$(ts.rows).each(function(i) {
						if (i>0) {
							if (rowIdRec && $.inArray(this.id,rowIdRec) > -1) {  //当不在排除列中
								if(!$(this).hasClass("ui-subgrid") && !$(this).hasClass("jqgroup") && !$(this).hasClass('ui-state-disabled')){
									$("#jqg_"+$.jgrid.jqID(ts.p.id)+"_"+$.jgrid.jqID(this.id) )[ts.p.useProp ? 'prop': 'attr']("checked",true);
									$(this).addClass("ui-state-highlight").attr("aria-selected","true");  
									ts.p.selarrrow.push(this.id);
...
//			lixj 修改，去掉原有的a标签拦截，即点击a标签仍然可以选中行，但同时带来的问题就是他也会触发onselect事件	
			if ((ts.p.aTagIntercept===false && td.tagName == 'A') || ((td.tagName == 'INPUT' || td.tagName == 'TEXTAREA' || td.tagName == 'OPTION' || td.tagName == 'SELECT' ) && !scb) ) { return; }
			if(cSel === true) {
				ri = ptr[0].id;
				...
				//默认点击行，而非点击checkbox的时候都触发onclick事件
				if(!scb){
					$(ts).jqGrid("clickRow",ri,true,e);
				}
				if(ts.p.clickUnselect && ts.p.multiselect){//当表格多选的时候，且点击和选择分离
					if(scb){
						if(ts.p.checkboxSingle){
				...
				}else if(ts.p.cellEdit === true) { //当表格单元可编辑时
				...
				} else if ( !ts.p.multikey ) { //当表格不存在多选的按键时
					...
					if(e[ts.p.multikey]) {//当表格存在多选的按键时
						$(ts).jqGrid("setSelection",ri,true,e);
		...
		$(grid.bDiv)
			.append($('<div style=" '+ts.p.bodyDivStyle+' position:relative;'+(isMSIE && $.browser.version < 8 ? "" : "")+'"></div>').append('<div></div>').append(this))
		...
		if( ts.p.toolbar[0] ) {
			...
			if(ts.p.toolbar[1]=="both") {
				...
				$(grid.uDiv).insertBefore(grid.hDiv).addClass("ui-userdata ui-state-default").attr("id","t_"+this.id).css({ height: 22+"px",padding: 2+"px"});//lxq 25to22
				$(grid.ubDiv).insertAfter(grid.hDiv).addClass("ui-userdata ui-state-default").attr("id","tb_"+this.id).css({ height: 22+"px",padding: 2+"px"});//lxq 25to22
				$(ts).jqGrid("addToToolBar","t_"+this.id,ts.p.mytoolbar);
				$(ts).jqGrid("addToToolBar","tb_"+this.id,ts.p.mytoolbar);
				if(hg)  {$(grid.ubDiv).hide();}
			} else {
				$(grid.uDiv).width(grid.width).addClass("ui-userdata ui-state-default").attr("id","t_"+this.id).css({ height: 22+"px",padding: 2+"px"});//lxq 25to22
				$(ts).jqGrid("addToToolBar","t_"+this.id,ts.p.mytoolbar);
			}
...
$.jgrid.extend({
	...
	getClickRowId:function(){
		var ts = $(this)[0];
		if(ts.p.clickUnselect && ts.p.multiselect){
			var clickRowSelection = $(this).data("clickRowSelection");
			return clickRowSelection;
		}
		return $(ts).jqGrid("getGridParam", 'selrow');
	},
	getClickRowData:function(){
		var ts = this;
		var id = $(ts).jqGrid('getClickRowId'); 
		var result = $(ts).jqGrid("getOriRowData", id);
		return result;
	},
	clickRow:function(selection,onsr,e){//新增点选行方法，参数同于选中行
		var before_selection = $(this).data("clickRowSelection");
		$(this).data("clickRowSelection",selection);
		return this.each(function(){
			...
			if(before_selection !== undefined){
				var before_tr = $t.rows.namedItem(before_selection+"");
				if($(before_tr).hasClass("datagrid-row-sel")){
					$(before_tr).removeClass("datagrid-row-sel");
				}
				if(fid) {
					$("#"+$.jgrid.jqID(before_selection), "#"+$.jgrid.jqID(fid)).removeClass("datagrid-row-sel");
				}
			}
			$(pt).addClass("datagrid-row-sel");
			if(fid) {
				$("#"+$.jgrid.jqID(selection), "#"+$.jgrid.jqID(fid)).addClass("datagrid-row-sel");
			}
			if( $t.p.onClickRow && onsr) { 
				var rowData = $($t).jqGrid("getOriRowData",pt.id);
				$t.p.onClickRow.call($t, pt.id,rowData, e); 
			}
		});
	},
	clickDefaultRow:function(onsr,e){
		var dataIds = $(this).jqGrid("getDataIDs"); 
		if(dataIds.length == 0){
			return;
		}
		var before_selection = $(this).data("clickRowSelection"),selection = dataIds[0];
		$(this).data("clickRowSelection",selection);
		return this.each(function(){
			var $t = this, stat,pt, ner, ia, tpsr, fid;
			if(selection === undefined) { return; }
			...
			if(!$t.p.multiselect){
				$($t).jqGrid("setSelection",selection,true,e);
			}
		});
	},
	selectRow:function(rowIndex){
		var ts = this;
		var dataIds = $(ts).jqGrid("getDataIDs"); 
		if(rowIndex < dataIds.length){
			$(ts).jqGrid("setSelection",dataIds[rowIndex],true); 
		}
	},
	 ...
	 * chen_xk
	 */
	getOriRowData:function(rowId){
		var res = {};
		this.each(function(){
			res = this.originalData[rowId];
		});
		return res;
	},
	getRowDataAll:function(){
		var res = {};
		this.each(function(){
			res = this.rows;
		});
	...
	getSelRowObjArr:function(){
		var res = new Array();
		this.each(function(){
			var $t = this;
			for(var index=0;index<$t.p.selarrrow.length;index++){
				res.push(this.originalData[$t.p.selarrrow[index]]);
			}
		});
		return res;
	},
	 ...
	 * chen_xk
	 */
	getOriRowDataAll:function(){
		var res;
		this.each(function(){
			res = this.originalData;
		});
		return res;
	},
		...
		if(rdata) {
			...
			this.each(function() {
				...
				while(k < datalen) {
					...
					row.unshift( t.constructTr(rowid, false, cna, lcdata, data, false ,i) );//为了添加样式
					row[row.length] = "</tr>";
					if(t.rows.length === 0){
	...
	setGridWidth : function(nwidth, shrink) {
		return this.each(function(){
			if (!this.grid ) {return;}
			var $t = this, cw,initwidth = 0, brd=$t.p.cellLayout, lvc, vc=0, hs=false, scw=$t.p.scrollOffset, aw, gw=0,
			cl = 0,cr;
	...
	querydata : function(id,data){
//	    jQuery("#" + id).setGridParam({"postData":""});
//	    jQuery("#" + id).setGridParam({"postData":data}).trigger("reloadGrid",[{page:1}]);
		this.queryGrid(data,null);
	},
	//data:查询条件，opt：grid参数
	queryGrid : function(data,opt){
		if(data){
			data["queryTotal"] = data["queryTotal"]==undefined?true:data["queryTotal"];
		}
		var param = {postData:data,page:1,"datatype":"json"};
		if(opt){
			$.extend(param,opt);
		}
		//清除原有参数，请不要删除 @chen_xk
		$($(this)[0]).setGridParam({"postData":""});
		$($(this)[0]).jqGrid("setGridParam",param).trigger("reloadGrid");
	},
	//重置toolbar，并移除menu中的按钮,如果按钮被全部移除，则隐藏toolbar
	hideToolMenu: function(menus){
		 this.each(function(){
			 var $t = this;
			 $($t).jqGrid("refreshToolBar","t_"+$t.id,$t.p.mytoolbar,menus);
		 });
	}
});
})(jQuery);
...
$.jgrid.extend({
	...
	setGroupHeaders : function ( o ) {
		...
		return this.each(function(){
			...
			cml = colModel.length,
			ths = ts.grid.headers,
			$htable = $("table.ui-jqgrid-htable", ts.grid.hDiv).css("height","100%"),
			...
			$(ts).prepend($thead);
			$tr = $('<tr>', {role: "rowheader"}).addClass("ui-jqgrid-labels jqg-third-row-header").css("height","100%");
			for (i = 0; i < cml; i++) {
				th = ths[i].el;
				$th = $(th);
				cmi = colModel[i];
	...
	setFrozenColumns : function () {
		return this.each(function() {
			...
			if( maxfrozen>=0 && frozen) {
				...
				//liugc修改叠加行高 lxq注释了下面的自动算法改为固定值
				$t.grid.fhDiv = $('<div style="position:absolute;left:0px;top:'+top+'px;height:'+hth+'px;background: none;" class="frozen-div ui-state-default ui-jqgrid-hdiv"></div>');
				$t.grid.fbDiv = $('<div style="position:absolute;left:0px;top:'+(parseInt(top,10)+parseInt(hth,10) + 1)+'px;overflow-y:hidden" class="frozen-bdiv ui-jqgrid-bdiv"></div>');
				
				//$t.grid.fhDiv = $('<div style="position:absolute;left:0px;top:'+(top-28)+'px;height:'+hth+'px;" class="frozen-div ui-state-default ui-jqgrid-hdiv"></div>');
				//$t.grid.fbDiv = $('<div style="position:absolute;left:0px;top:'+(parseInt(top,10)+parseInt(hth,10) - 27)+'px;overflow-y:hidden" class="frozen-bdiv ui-jqgrid-bdiv"></div>');
//				$t.grid.fhDiv = $('<div style="position:absolute;left:0px;top:30px;" class="frozen-div ui-state-default ui-jqgrid-hdiv"></div>');
//				$t.grid.fbDiv = $('<div style="position:absolute;left:0px;top:57px;overflow-y:hidden" class="frozen-bdiv ui-jqgrid-bdiv"></div>');
...
$.jgrid.extend({
	...
	addToToolBar:function(id,toolbar){
		...
		if (toolbar) {
			...
			for(var i=0; i<toolbar.length; i++) {
				var btn = toolbar[i];
				var divO = $('<div class="btn-primary_wk" id="div_'+btn.id+'"></div>'),
					divL = $('<div class="btn-primary_wk_l">&nbsp;</div>'),
					divM = $('<div class="btn-primary_wk_m"></div>'),
					divR = $('<div class="btn-primary_wk_r">&nbsp;</div>'),
	...
	refreshToolBar:function(id,toolbar,menus){
		var tb = $("#" + id),toolbar_show = [],i,btn;
		tb.empty();
		for(i=0;i<toolbar.length; i++){
			if(!$.isArray(menus) || $.inArray(toolbar[i].id, menus)<0){
				toolbar_show.push(toolbar[i]);
			}
		}
		if(toolbar_show.length == 0){//当菜单的个数等于要移除菜单的个数时，隐藏工具栏
			$("#" + id).css("display","none");
			return;
	  	}
		$("#" + id).css("display","");
		if (toolbar_show) {
			for(i=0; i<toolbar_show.length; i++) {
				btn = toolbar_show[i],divO = $('<div class="btn-primary_wk" id="div_'+btn.id+'"></div>'),
					divL = $('<div class="btn-primary_wk_l">&nbsp;</div>'),
					divM = $('<div class="btn-primary_wk_m"></div>'),
					divR = $('<div class="btn-primary_wk_r">&nbsp;</div>'),
					tool = $('<input type="button">');
  ...
  function loopy() {
    ...
    timeout_id = window[ str_setTimeout ](function(){
      ...
      elems.each(function(){
        ...
        // data store and trigger the 'resize' event. 增加宽度和高度变化的参数数
        var wc= (width !== data.w);
        var hc = (height !== data.h);
        if ( wc || hc ) {
          elem.trigger( str_resize, [ data.w = width, data.h = height,data.wc = wc,data.hc=hc ] );
        }
